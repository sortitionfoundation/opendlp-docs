...
2. For each of the entries in the **category** column of the categories file, there is a column in the respondents file with that header.
   - E.g. in the sample categories file, the category column contains five entries: *gender, age\_bucket, ethnicity, disability, colour\_bucket*. In the sample respondents file there are five columns with these headers.
3. For each column in the respondents file that corresponds to a stratification category, every single entry below the header must correspond to one of the listed values from the categories file.
   ...
   - E.g. Likewise in our sample category file, we had an *age\_bucket* category with values *0-17 / 18-24 / 35-34 / etc.* If you look at column K in our sample respondents file, you will see that its header is gender and every entry below the header corresponds to one of the values listed in the category file.
